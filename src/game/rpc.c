 ...
 * 
 *    @return u32                            1 if the relationship is a friend, 0 otherwise.
 */
u32 rpc_relationship_pass_filter( void *spData, struct DiscordRelationship *spRelationship ) {
    return ( spRelationship->type == DiscordRelationshipType_Friend );
}

/*
 *    Returns if the relationship is a friend and the user is less cool than the pApplication.
 *    ... ok copilot...
 ...
 *    @param struct DiscordRelationship *    The relationship.
 * 
 *    @return u32                            1 if the relationship is a friend and the user is less cool than the pApplication, 0 otherwise.
 */
u32 rpc_relationship_snowflake_filter( void *spData, struct DiscordRelationship *spRelationship ) {
...
void rpc_update_status( void *spData ) {
    ...
    pApp->apActivities->update_activity( pApp->apActivities, &activity, pApp, rpc_update_activity_callback );
}
 ...
 */
void rpc_oauth2_callback( void *spData, enum EDiscordResult sResult, struct DiscordOAuth2Token *spToken )
{
 ...
 */
void rpc_lobby_callback( void *spData, enum EDiscordResult sResult, struct DiscordLobby *spLobby )
{
...
void setup_rpc( void ) {
    ...
    gDiscord.apAchievements = gDiscord.apCore->get_achievement_manager( gDiscord.apCore );
    gDiscord.apActivities   = gDiscord.apCore->get_activity_manager( gDiscord.apCore );
    ...
    gDiscord.apLobbies->connect_lobby_with_activity_secret( gDiscord.apLobbies, "invalid_secret", &gDiscord, &rpc_lobby_callback );

    gDiscord.apApp->get_oauth2_token( gDiscord.apApp, &gDiscord, &rpc_oauth2_callback );

    DiscordBranch branch;
    gDiscord.apApp->get_current_branch( gDiscord.apApp, &branch );
    log_note( "Discord branch: %s\n", branch );

    gDiscord.apRelationships = gDiscord.apCore->get_relationship_manager( gDiscord.apCore );
}
